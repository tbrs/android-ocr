 ...
 */
internal class CaptureActivityHandler(private val activity: CaptureActivity, private val cameraManager: CameraManager?, isContinuousModeActive: Boolean) : Handler() {
    ...
    override fun handleMessage(message: Message) {
        ...
        when (message.what) {
            R.id.restart_preview -> restartOcrPreview()
            R.id.ocr_continuous_decode_failed -> {
                DecodeHandler.resetDecodeState()
                try {
                    activity.handleOcrContinuousDecode(message.obj as OcrResultFailure)
            ...
            R.id.ocr_continuous_decode_succeeded -> {
                DecodeHandler.resetDecodeState()
                try {
                    activity.handleOcrContinuousDecode(message.obj as OcrResult)
                } catch (e: NullPointerException) {
                    // Continue
                }
            ...
            R.id.ocr_decode_succeeded -> {
                state = State.SUCCESS
                activity.setShutterButtonClickable(true)
                activity.handleOcrDecode(message.obj as OcrResult)
            }
            R.id.ocr_decode_failed -> {
                state = State.PREVIEW
                activity.setShutterButtonClickable(true)
                val toast = Toast.makeText(activity.baseContext, "OCR failed. Please try again.", Toast.LENGTH_SHORT)
     ...
     */
    private fun restartOcrPreviewAndDecode() {
     ...
     */
    fun hardwareShutterButtonClick() {
