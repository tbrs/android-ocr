 ...
 */
class Translator private constructor(activity: Activity)// Private constructor to enforce noninstantiability
{
    companion object {

        val BAD_TRANSLATION_MSG = "[Translation unavailable]"

        internal fun translate(activity: Activity, sourceLanguageCode: String, targetLanguageCode: String, sourceText: String): String {
            var sourceLanguageCode = sourceLanguageCode

            // Check preferences to determine which translation API to use--Google, or Bing.
            val prefs = PreferenceManager.getDefaultSharedPreferences(activity)
            val api = prefs.getString(PreferencesActivity.KEY_TRANSLATOR, CaptureActivity.DEFAULT_TRANSLATOR)

            // Delegate the translation based on the user's preference.
            if (api == PreferencesActivity.TRANSLATOR_BING) {

                // Get the correct code for the source language for this translation service.
                sourceLanguageCode = TranslatorBing.toLanguage(
                        LanguageCodeHelper.getTranslationLanguageName(activity.baseContext, sourceLanguageCode))

                return TranslatorBing.translate(sourceLanguageCode, targetLanguageCode, sourceText)
            } else if (api == PreferencesActivity.TRANSLATOR_GOOGLE) {
